...
public class Controller17MultiAccionesMates extends MultiActionController {
    ...
    public ModelAndView tablamultiplicar(HttpServletRequest request, HttpServletResponse response) {
        ModelAndView mv = new ModelAndView("web17multiaccionesmates");
    ...
    @RequestMapping(method = RequestMethod.POST)
    public ModelAndView numerospares(HttpServletRequest request, HttpServletResponse response) {
        ModelAndView mv = new ModelAndView("web17multiaccionesmates");
        ArrayList<Integer> resultado = new ArrayList<>();
        for (int i = 1; i <= 10; i++) {
            if (i % 2 == 0) {
